...
import { LayoutDashboard, Briefcase, Calendar, MessageSquare, Settings, X, Zap } from "lucide-react";
...
export default function Sidebar({ isOpen, setIsOpen }: { isOpen?: boolean; setIsOpen?: (v: boolean) => void }) {
  ...
  return (
    <>
      ...
      */}
      <aside className={`bg-white border-r border-slate-200 h-screen flex flex-col fixed left-0 top-0 z-50 transition-all duration-300 ease-in-out group ${
        isOpen ? 'translate-x-0 w-64' : '-translate-x-full md:translate-x-0 w-64 md:w-20 hover:w-64'
      }`}>
        {/* Brand */}
        <div className="h-16 flex items-center justify-between px-5 border-b border-slate-100 overflow-hidden whitespace-nowrap">
          <div className="flex items-center gap-3 text-xl font-bold bg-gradient-to-r from-indigo-500 to-violet-500 bg-clip-text text-transparent">
            ...
            <span className={`transition-opacity duration-300 ${isOpen ? 'opacity-100' : 'md:opacity-0 group-hover:opacity-100'}`}>
              FutureJob
            </span>
          </div>
          <button 
        ...
        <nav className="flex-1 px-3 py-6 space-y-2 overflow-x-hidden overflow-y-auto">
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                <item.icon className={`w-6 h-6 shrink-0 ${isActive ? 'text-indigo-600' : ''}`} strokeWidth={isActive ? 2.5 : 2} />
                <span className={`transition-opacity duration-300 ${isOpen ? 'opacity-100' : 'md:opacity-0 group-hover:opacity-100'}`}>
                  {item.name}
                </span>
              </Link>
